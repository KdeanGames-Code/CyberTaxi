/**
 * @file server/routes/auth/authRoutes.js
...
router.post("/login/username", async (req, res) => {
    try {
        ...
        // Generate JWT token
        const token = generateToken({ player_id: player.player_id });
        console.log("User authenticated"); // Success log
...
router.post("/reset-password", authenticateJWT, async (req, res) => {
    try {
        ...
        if (player.player_id !== req.user.player_id) {
            console.log(`Password reset failed: Unauthorized access for player_id: ${req.user.player_id}`);
